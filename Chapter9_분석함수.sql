...
SELECT ORD.CUS_ID
    , SUM(ORD.ORD_AMT) ORD_AMT
    , ROW_NUMBER() OVER(ORDER BY SUM(ORD.ORD_AMT) DESC) RNK
    , LAG(ORD.CUS_ID, 1) OVER(ORDER BY SUM(ORD.ORD_AMT) DESC) LAD_1
    , LEAD(ORD.CUS_ID, 1) OVER(ORDER BY SUM(ORD.ORD_AMT) DESC) LEAD_1
FROM T_ORD ORD
WHERE ORD.ORD_DT >= TO_DATE('20170301', 'YYYYMMDD')
