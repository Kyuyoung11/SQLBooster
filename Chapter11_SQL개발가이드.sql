...
SELECT  T1.SQL_ID ,T1.CHILD_NUMBER ,T1.SQL_TEXT, T1.LAST_ACTIVE_TIME
FROM    V$SQL T1
WHERE   T1.SQL_TEXT LIKE '%SELECT  T1. CUS_ID ,COUNT(*) ORD_CNT%'
ORDER BY T1.LAST_ACTIVE_TIME DESC;


SELECT  *
FROM    TABLE(DBMS_XPLAN.DISPLAY_CURSOR('7jp29f0aszs75',0,'ALLSTATS LAST'));
...
SELECT  *
FROM    TABLE(DBMS_XPLAN.DISPLAY_CURSOR('auqhb8zajk8zk',0,'ALLSTATS LAST'));
...
SELECT  T1.SQL_ID ,T1.CHILD_NUMBER ,T1.SQL_TEXT, T1.LAST_ACTIVE_TIME
FROM    V$SQL T1
WHERE   T1.SQL_TEXT LIKE '%T1.PAY_TP ,COUNT(*) CNT%'
ORDER BY T1.LAST_ACTIVE_TIME DESC;


SELECT  *
FROM    TABLE(DBMS_XPLAN.DISPLAY_CURSOR('au7kfkzv6jj8c',0,'ALLSTATS LAST'));
...
SELECT  T1.SQL_ID ,T1.CHILD_NUMBER ,T1.SQL_TEXT, T1.LAST_ACTIVE_TIME
FROM    V$SQL T1
WHERE   T1.SQL_TEXT LIKE '%INDEX(T1 X_T_ACC_TRN_1)%'
ORDER BY T1.LAST_ACTIVE_TIME DESC;


SELECT  *
FROM    TABLE(DBMS_XPLAN.DISPLAY_CURSOR('0mz1umyqvu2sx',0,'ALLSTATS LAST'));
