...
WHERE CUS.CUS_ID IN ('CUS_0003', 'CUS_0004')
AND ORD.ORD_DT >= TO_DATE('20170201', 'YYYYMMDD')
AND ORD.ORD_DT < TO_DATE('20170501', 'YYYYMMDD')
GROUP BY CUS.CUS_ID, CUS.CUS_NM, TO_CHAR(ORD.ORD_DT, 'YYYYMM')
ORDER BY CUS.CUS_ID, CUS.CUS_NM
;
...
FROM (SELECT CUS.CUS_ID
           ...
           , YM.BASE_YM
      FROM M_CUS CUS
               CROSS JOIN (SELECT TO_CHAR(ADD_MONTHS(TO_DATE('20170201', 'YYYYMMDD'), ROWNUM - 1), 'YYYYMM') BASE_YM
                           FROM DUAL
                           CONNECT BY ROWNUM <= 3) YM
     WHERE CUS.CUS_ID IN ('CUS_0003', 'CUS_0004')
    ) BASE
    LEFT OUTER JOIN T_ORD ORD
        ON ORD.CUS_ID = BASE.CUS_ID
        AND ORD.ORD_DT >= TO_DATE('20170201', 'YYYYMMDD')
        AND ORD.ORD_DT < TO_DATE('20170501', 'YYYYMMDD')
        AND BASE.BASE_YM = TO_CHAR(ORD.ORD_DT, 'YYYYMM')
GROUP BY BASE.CUS_ID, BASE.BASE_YM
ORDER BY BASE.CUS_ID, BASE.BASE_YM
;
...
SELECT CUS.CUS_GD
    ,ITM.ITM_TP
    ,COUNT(ORD.ORD_SEQ) ORD_QTY
FROM M_CUS CUS
    LEFT OUTER JOIN T_ORD ORD
        ON CUS.CUS_ID = ORD.CUS_ID
        AND ORD.ORD_ST = 'COMP'
    INNER JOIN T_ORD_DET ORDT
        ON ORDT.ORD_SEQ = ORD.ORD_SEQ
    LEFT OUTER JOIN M_ITM ITM
        ON ITM.ITM_ID = ORDT.ITM_ID
...
SELECT BASE.CUS_GD
    ,BASE.ITM_TP
    ,COUNT(DATA.ITM_ID) ORD_QTY
FROM (SELECT CUS.CUS_ID
          , ITM.ITM_ID
          , CUS.CUS_GD
          , ITM.ITM_TP
    FROM M_CUS CUS
    CROSS JOIN M_ITM ITM
    ) BASE
    LEFT OUTER JOIN (
        SELECT ORD.CUS_ID
            , ORDT.ITM_ID
        ...
        INNER JOIN T_ORD_DET ORDT
            ON ORDT.ORD_SEQ = ORD.ORD_SEQ
        WHERE ORD.ORD_ST = 'COMP'
    ) DATA
    ON DATA.CUS_ID = BASE.CUS_ID
    AND DATA.ITM_ID = BASE.ITM_ID
GROUP BY BASE.CUS_GD, BASE.ITM_TP
ORDER BY BASE.CUS_GD, BASE.ITM_TP
;
